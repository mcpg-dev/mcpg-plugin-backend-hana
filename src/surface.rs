...
mod tests {
    ...
    fn static_uri_wins_over_argument() {
        let args = json!({ "uri": "hana://from-arg" });
        assert_eq!(
            resolve_resource_uri(Some("hana://static"), &args),
            Some("hana://static")
        );
    }
    ...
    fn falls_back_to_argument_uri() {
        let args = json!({ "uri": "hana://docs/readme" });
        assert_eq!(
            resolve_resource_uri(None, &args),
            Some("hana://docs/readme")
        );
    }
    ...
    fn resource_body_satisfies_decoder_shape() {
        ...
        let body = resource_contents_body("hana://docs", &rows);
        let contents = body["contents"].as_array().expect("contents array");
        assert_eq!(contents.len(), 1);
        assert_eq!(contents[0]["uri"], json!("hana://docs"));
    ...
    fn rows_map_to_resources_with_optional_columns() {
        let rows = vec![
            json!({ "uri": "hana://a", "name": "A", "description": "first", "mime_type": "text/plain", "id": 1 }),
            json!({ "uri": "hana://b", "id": 2 }),
        ];
        let page = rows_to_resource_page(&rows, &keyset_cfg(10), 0);
        assert_eq!(page.resources.len(), 2);
        assert_eq!(page.resources[0].uri, "hana://a");
    ...
    fn full_keyset_page_carries_cursor() {
        let rows = vec![json!({ "uri": "hana://a", "id": 7 })];
    ...
    fn offset_full_page_advances_offset() {
        ...
        let rows = vec![json!({ "uri": "hana://a" }), json!({ "uri": "hana://b" })];
    ...
    fn rows_without_uri_are_skipped() {
        let rows = vec![json!({ "name": "no uri" }), json!({ "uri": "hana://ok" })];
        let page = rows_to_resource_page(&rows, &keyset_cfg(10), 0);
        assert_eq!(page.resources.len(), 1);
        assert_eq!(page.resources[0].uri, "hana://ok");
    }
